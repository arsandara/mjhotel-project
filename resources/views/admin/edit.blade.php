...
<html lang="id">
  <head>
    ...
    <title>Edit Kamar | Hotel Mukti Jaya</title>
    <link
    ...
    * {
    ...
    /* PERBAIKAN: Batasi lebar card */
    .card {
      background: #fff;
      border: 1px solid var(--border);
      border-radius: 16px;
      box-shadow: var(--shadow);
      max-width: 1000px; /* Diperpanjang dari 800px */
      width: 100%;
    }
    .card-b {
      padding: 18px;
    ...
    .form {
      ...
      gap: 16px;
      max-width: 100%; /* Pastikan form tidak melebihi card */
    }
    .field {
      display: flex;
      flex-direction: column;
    ...
    /* PERBAIKAN: Actions tidak melebihi card */
    .actions {
      display: flex;
      gap: 10px;
      ...
      max-width: 100%; /* Pastikan tidak melebihi card */
    }
    .btn {
      height: 40px;
      border: 0;
  ...
  <body>
    <div class="app">
      ...
      <main>
        ...
        <p class="subtitle">
          Ubah data kamar {{ $room->room_name }}
        </p>
        ...
        <section class="card">
          <form id="roomForm" method="POST" action="{{ route('admin.rooms.update', $room->room_id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-b">
              <div class="form">
                ...
                <div class="two-col">
                  ...
                  <div class="field">
                    ...
                    <select name="room_type" required>
                      ...
                      <option value="Superior" {{ old('room_type', $room->room_type) == 'Superior' ? 'selected' : '' }}>Superior</option>
                ...
                <div class="two-col">
                  <div class="field">
                    ...
                    <input type="number" name="room_price" value="{{ old('room_price', $room->room_price) }}" min="0" required />
                  </div>
                  <div class="field">
                    <label>Kapasitas (Orang) *</label>
                    <input type="number" name="room_capacity" value="{{ old('room_capacity', $room->room_capacity) }}" min="1" required />
                  </div>
                </div>
                ...
                <div class="field">
                  ...
                  <textarea name="room_facility" placeholder="WiFi gratis, TV kabel, AC, kamar mandi dalam, air panas, dll.">{{ old('room_facility', $room->room_facility) }}</textarea>
                </div>
                ...
                <div class="field">
                  ...
                  <div class="existing-images" id="existingImages">
                    ...
                    <div class="image-item" data-id="{{ $image->id }}">
                      <img src="{{ asset('images/rooms/' . $image->image_path) }}" alt="Room image"/>
                      <button type="button" class="delete-existing-image" onclick="deleteExistingImage({{ $image->id }})">×</button>
                    </div>
                    @endforeach
                  </div>
                </div>
    ...
    <script>
      ...
      let currentPhotosCount = {{ $room->images->count() }};
      let selectedFiles = [];
      ...
      function addExistingImagesDragAndDrop() {
          const existingImages = document.querySelectorAll('#existingImages .image-item');
          existingImages.forEach((image) => {
              ...
              image.addEventListener('drop', function(e) {
                  ...
                  if (draggedItem !== this) {
                      ...
                      const fromIndex = allImages.indexOf(draggedItem);
                      const toIndex = allImages.indexOf(this);
      ...
      async function handleFileSelect(e) {
          ...
          for (const f of files) {
              ...
              if (currentPhotosCount + selectedFiles.length >= MAX_PHOTOS) {
      ...
      async function deleteExistingImage(imageId) {
          ...
          try {
              const response = await fetch(`/admin/rooms/image/${imageId}/delete`, {
                  method: 'POST',
                  headers: {
              ...
              if (data.success) {
                  ...
                  currentPhotosCount--;
                  if (currentPhotosCount < MAX_PHOTOS && !gallery) createUploadSection();
                  else if (gallery) drawGallery();
      ...
      function createUploadSection() {
          if (currentPhotosCount + selectedFiles.length >= MAX_PHOTOS) return;
          document.getElementById('uploadSectionContainer').innerHTML = `
              <div class="field" id="uploadSection">
                  <label>Tambah Foto Baru</label>
                  <p class="upload-info">📷 Maksimal 5 foto total • 📏 Maksimal 2MB per file</p>
      ...
      function handleFormSubmit(e) {
          ...
          // Kirim urutan existing images
          const existingOrder = [];
          const existingImgs = document.querySelectorAll('#existingImages .image-item');
          ...
          formData.append('existing_order', JSON.stringify(existingOrder));
          
          // Debug log
          console.log('Sending order:', existingOrder);
          ...
          fetch('/admin/rooms/{{ $room->room_id }}', {
              method: 'POST',
              headers: {
                  'X-CSRF-TOKEN': CSRF_TOKEN,
      ...
      document.addEventListener('DOMContentLoaded', function() {
          addExistingImagesDragAndDrop();
          if (currentPhotosCount < MAX_PHOTOS) createUploadSection();
          const form = document.getElementById('roomForm');
